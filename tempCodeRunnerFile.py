...
def get_student_groups(current_user, app_config):
    """Helper function to group students by class and area for dashboard display"""
    students_by_class = {}
    students_by_area = {}
    ...
    if hasattr(current_user, 'is_teacher') and current_user.is_teacher():
        assigned_areas = current_user.get_assigned_study_areas(app_config)
        if assigned_areas:
    ...
    for s in student_query.all():
        ...
        # Ensure we're always appending to a list
        if not isinstance(students_by_class[cls], list):
            students_by_class[cls] = []
        students_by_class[cls].append(s)
        area = s.get_study_area_display() or 'Unspecified'
        if area not in students_by_area:
            students_by_area[area] = 0
        students_by_area[area] = students_by_area.get(area, 0) + 1
    
    return students_by_class, students_by_area
...
def dashboard():
    ...
    # For teachers, show only their assessments and student summaries for their assigned areas
    ...
    if hasattr(current_user, 'is_teacher') and current_user.is_teacher():
        ...
        # Get student summaries for teacher's assigned study areas
        assigned_areas = current_user.get_assigned_study_areas(app.config)
        if assigned_areas and current_user.subject:
            # Get all students in assigned study areas who have assessments in teacher's subject by this teacher
            students_with_assessments = db.session.query(Student).join(Assessment)\
                .filter(Student.study_area.in_(assigned_areas))\
...
def students():
    ...
    if hasattr(current_user, 'is_teacher') and current_user.is_teacher():
        assigned_areas = current_user.get_assigned_study_areas(app.config)
        if assigned_areas:
            query = query.filter(Student.study_area.in_(assigned_areas))
    ...
    if group_by == 'class':
        grouped_students = {}
        for student in students:
            key = student.get_class_display() or 'Unspecified'
            grouped_students.setdefault(key, []).append(student)
    elif group_by == 'study_area':
        grouped_students = {}
        for student in students:
            key = student.get_study_area_display() or 'Unassigned'
            grouped_students.setdefault(key, []).append(student)
    else:
    ...
    for group_name, group_students in grouped_students.items():
        if sort_by == 'name':
            sorted_students = sorted(group_students, key=lambda s: (s.last_name or '', s.first_name or ''))
        elif sort_by == 'class':
            sorted_students = sorted(group_students, key=lambda s: s.get_class_display() or '')
        elif sort_by == 'study_area':
            sorted_students = sorted(group_students, key=lambda s: s.get_study_area_display() or '')
...
def student_bulk_import():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            # Normalize imported class and study area values
...
def user_bulk_import():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            users_data = importer.import_teachers()
            ...
            for data in users_data:
                ...
                password = data.get('password') or app.config.get('DEFAULT_STUDENT_PASSWORD', 'Teacher@123')
                password = password.strip()
                pw_hash = bcrypt.generate_password_hash(password).decode('utf-8')

                subject_key = canonical_subject_key(data.get('subject'))
                classes_value = data.get('classes')
                class_keys = []
                if classes_value:
                    raw_classes = re.split(r'[;,]', classes_value)
                    for raw_class in raw_classes:
                        canonical_class = canonical_class_key(raw_class)
                        if canonical_class:
                            class_keys.append(canonical_class)
                ...
                if class_keys:
                    user.set_classes_list(sorted(set(class_keys)))
...
def assessments_list():
    ...
    if hasattr(current_user, 'is_teacher') and current_user.is_teacher():
        ...
        # Also filter to only students in assigned study areas
        assigned_areas = current_user.get_assigned_study_areas(app.config)
        if assigned_areas:
            query = query.join(Student).filter(Student.study_area.in_(assigned_areas))
    else:
    ...
    for teacher in teachers:
        if teacher.subject:
            ...
            teacher_assignments[teacher.id] = {
                ...
                'assigned_classes': teacher.get_classes_list()
            }
    
    # Get unassigned classes (study areas not covered by any teacher)
    all_study_areas = [area[0] for area in study_areas]
    assigned_areas = set()
    for assignment in teacher_assignments.values():
        assigned_areas.update(assignment['assigned_areas'])
    
    unassigned_areas = [area for area in all_study_areas if area not in assigned_areas]
...
def download_template(template_type):
    ...
    elif template_type == "user_import":
        template_path = os.path.join(app.config['TEMPLATE_FOLDER'], 'user_import_template.xlsx')
        filename = "teacher_bulk_import_template.xlsx"
    else:
        abort(404)
    ...
    elif template_type == "user_import" and not os.path.exists(template_path):
        create_teacher_import_template(template_path)
