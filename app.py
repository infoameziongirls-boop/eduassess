...
# STEP 1: Grab SECRET_KEY from environment BEFORE creating the app.
# Write it back to os.environ so the config class picks it up too.
_sk = os.environ.get('SECRET_KEY', '').strip()
if not _sk:
    _sk = 'fallback-dev-secret-do-not-use-in-production-xyz123abc456'
    print("[WARNING] SECRET_KEY not found in environment. Using fallback.")
os.environ['SECRET_KEY'] = _sk

# STEP 2: Create the app
app = Flask(__name__, static_folder='public')

# STEP 3: Load config object
env = os.environ.get('FLASK_ENV', 'development')
app.config.from_object(config[env])

# STEP 4: Force SECRET_KEY AGAIN after from_object() to prevent it being overwritten
app.secret_key = _sk
app.config['SECRET_KEY'] = _sk
print(f"[OK] SECRET_KEY set, length={len(_sk)}")

# STEP 5: Persistent directory setup
...
# NEVER use SESSION_TYPE = 'null' — it breaks sessions and CSRF entirely.
# Use filesystem sessions in all environments.
# -------------------------
...
print(f"[OK] Session configured: filesystem ({env})")
...
Session(app)  # Must come AFTER session config is set

# STEP 8: Initialize database and migrate
init_db(app, bcrypt)
migrate = Migrate(app, db)

# STEP 9: Load persistent config from DB (after DB is ready)
...
# -------------------------
# STEP 10: User loader for Flask-Login
# -------------------------
