    ...
    def import_students(self, start_row=2):
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=start_row, values_only=True):
                if not any(row):
                    continue
                has_admission_id = len(row) >= 7
                student_data = {
                    'student_id_code': normalize(row[0]) if has_admission_id else None,
                    'student_number': normalize(row[1]) if has_admission_id else (normalize(row[0]) if len(row) > 0 else None),
                    'first_name':     normalize(row[2]) if has_admission_id else (normalize(row[1]) if len(row) > 1 else None),
                    'last_name':      normalize(row[3]) if has_admission_id else (normalize(row[2]) if len(row) > 2 else None),
                    'middle_name':    normalize(row[4]) if has_admission_id else (normalize(row[3]) if len(row) > 3 else None),
                    'class_name':     normalize(row[5]) if has_admission_id else (normalize(row[4]) if len(row) > 4 else None),
                    'study_area':     normalize(row[6]) if has_admission_id else (normalize(row[5]) if len(row) > 5 else None),
                }
...
def create_student_import_template(output_path):
    ...
    header_fill = PatternFill(start_color="4472C4", end_color="4472C4", fill_type="solid")
    header_font = Font(bold=True, color="FFFFFF", size=12)
    headers = ["Student ID (Admission Number)", "Student Number", "First Name", "Last Name", "Middle Name", "Class", "Study Area"]
    ...
    sample_data = [
        ["ZGS/HE26/001", "STU001", "John", "Doe", "Michael", "Form 1", "Home Economics A"],
        ["ZGS/GA26/001", "STU002", "Jane", "Smith", "", "Form 2", "General Arts 4B"],
        ["ZGS/BA26/001", "STU003", "Bob", "Johnson", "William", "Form 3", "Business A"],
    ]
    for row_idx, row_data in enumerate(sample_data, start=2):
        for col_idx, value in enumerate(row_data):
            ws.cell(row=row_idx, column=col_idx + 1, value=value)

    for idx, width in enumerate([25, 15, 15, 15, 15, 10, 20]):
